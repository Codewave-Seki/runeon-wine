...
- Current Dev source: [`cx26.3-wine11.0-runeon.11`](BACKPORTS-11.16-11.18-SWEEP.md) (Pre-release), paired with seed `2026.09.26` and MoltenVK `1.4.2`, App gate `>=1.8 (0)`. It preserves `.10` and adds the Wine 11.16–11.18 user-mode fix sweep, the winegstreamer video-pool fallback and two opt-in hooks (NW.js launch, DXGI driver version) that are inert unless the caller enables them. Select its `patchsets/` definition explicitly; the default series remains the historical `.9`.
- Current Production source and latest stable Release: `cx26.3-wine11.0-runeon.10` for seed `2026.09.22`, App gate `>=1.8 (0)`
- Validation: `.11` passed a complete x86_64/WoW64 build, signing, release readiness, and authenticated Dev feed/ticket/full-download checks for supported builds. Real Steam/game acceptance is pending the user’s Dev validation. `.10` remains the Production source; its Dev acceptance and Production authorization were recorded on 2026-09-23.
- Retained Production rollback sources: `cx26.3-wine11.0-runeon.9` for seed `2026.09.13`, `cx26.3-wine11.0-runeon.8` for seed `2026.09.06`, `cx26.3-wine11.0-runeon.6` for seed `2026.08.11.1`, and `cx26.3-wine11.0-runeon.5` for seed `2026.08.03.2`
...
- [`cx26.3-wine11.0-runeon.10`](https://github.com/Codewave-Seki/runeon-wine/releases/tag/cx26.3-wine11.0-runeon.10) is the stable corresponding-source Release for current Production seed `2026.09.22`.
- [`cx26.3-wine11.0-runeon.11`](https://github.com/Codewave-Seki/runeon-wine/releases/tag/cx26.3-wine11.0-runeon.11) is the Pre-release Dev candidate for seed `2026.09.26`. Immutable source details are in [the release manifest](release-manifests/cx26.3-wine11.0-runeon.11.source-archive.json).
