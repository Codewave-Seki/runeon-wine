...
- Default source definition (historical): `cx26.3-wine11.0-runeon.9`, paired with retained seed `2026.09.13` and MoltenVK `1.4.2`
- Current Dev and Production source: [`cx26.3-wine11.0-runeon.11`](BACKPORTS-11.16-11.18-SWEEP.md) (stable/latest), paired with seed `2026.09.26` and MoltenVK `1.4.2`, App gate `>=1.8 (0)`. It preserves `.10` and adds the Wine 11.16–11.18 user-mode fix sweep, the winegstreamer video-pool fallback and two opt-in hooks (NW.js launch, DXGI driver version) that are inert unless the caller enables them. Production promotes the exact signed Dev archive without rebuilding. Select its `patchsets/` definition explicitly; the default series remains the historical `.9`.
- Current Production source and latest stable Release: `cx26.3-wine11.0-runeon.11` for seed `2026.09.26`, App gate `>=1.8 (0)`
- Validation: `.11` passed a complete x86_64/WoW64 build, signing, release readiness, and authenticated Dev and Production feed/ticket/full-download checks for supported builds. The user reported no issues in Dev and authorized Production on 2026-09-26. This general acceptance does not establish independent coverage of every Steam/CEF, game or App interaction scenario.
- Retained Production rollback sources: `cx26.3-wine11.0-runeon.10` for seed `2026.09.22`, `cx26.3-wine11.0-runeon.9` for seed `2026.09.13`, `cx26.3-wine11.0-runeon.8` for seed `2026.09.06`, `cx26.3-wine11.0-runeon.6` for seed `2026.08.11.1`, and `cx26.3-wine11.0-runeon.5` for seed `2026.08.03.2`
- Upstream audited through: `wine-11.15` (complete audit); filtered sweep through `wine-11.18`
