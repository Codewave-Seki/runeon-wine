...
- Current Dev and Production source: [`cx26.3-wine11.0-runeon.10`](FLSGETVALUE2.md) (stable/latest), paired with seed `2026.09.22` and MoltenVK `1.4.2`, App gate `>=1.8 (0)`. Production promotes the exact signed Dev archive without rebuilding. Select the independent `patchsets/` definition explicitly; the default series remains the historical `.9`.
- Current Production source and latest stable Release: `cx26.3-wine11.0-runeon.10` for seed `2026.09.22`, App gate `>=1.8 (0)`
- Validation: API and isolated-prefix checks, signing and release readiness passed. The user reported no issues in Dev and authorized Production on 2026-09-23. This general acceptance does not establish independent coverage of every Steam/CEF, game or App interaction scenario. App installers and distribution verification are managed by the product repository.
