...
- Current shipped source: `cx26.3-wine11.0-runeon.6` for Dev seed `2026.08.11.1`; Production remains on `.5` / `2026.08.03.2` until promotion completes
- Stable source selected for Production promotion: `cx26.3-wine11.0-runeon.6`
...
- [`cx26.3-wine11.0-runeon.5`](https://github.com/Codewave-Seki/runeon-wine/releases/tag/cx26.3-wine11.0-runeon.5) is the stable corresponding-source Release for current Production and Dev rollback seed `2026.08.03.2`. It inherited `.4`, streamlined managed Steam launch validation, and remains immutable together with its published assets and checksums.
- [`cx26.3-wine11.0-runeon.6`](https://github.com/Codewave-Seki/runeon-wine/releases/tag/cx26.3-wine11.0-runeon.6) is the stable corresponding-source Release selected for Production promotion and already delivered in Dev seed `2026.08.11.1`. It inherits `.5` and adds nine reviewed backports from Wine 11.15, extending the audit through `wine-11.15`; its decisions and exclusions are recorded in [`AUDIT-11.0-11.15.md`](AUDIT-11.0-11.15.md). Build, signing, readiness, authenticated download, and product-path validation are complete; it becomes the Production source record only when the matching immutable seed is promoted.
...
To rebuild the exact source for retained legacy seed `2026.07.22`, select `.0` explicitly. Do not substitute any later patch set, including the active `.6` candidate:
