...
- 現在の active patch set: `cx26.3-wine11.0-runeon.1`
...
- `patchsets/cx26.3-wine11.0-runeon.0` は、Production seed `2026.07.22` に対応する正確な履歴ソース定義です。未公開の Escape `cfgmgr32` backport は含みません。既定の [`series`](series) は、引き続き次期 runtime の `.1` candidate を示します。
...
配布済み seed `2026.07.22` の正確なソースを再構築する場合は、`.0` を明示的に選択してください。Escape の修正を含む `.1` candidate で置き換えてはいけません。
