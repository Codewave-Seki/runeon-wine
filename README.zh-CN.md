...
- 当前源码候选：`cx26.3-wine11.0-runeon.4`
- 上游已审计至：`wine-11.14`
...
- `patchsets/cx26.3-wine11.0-runeon.0` 是 Production seed `2026.07.22` 的精确历史源码定义；`patchsets/cx26.3-wine11.0-runeon.1` 冻结了第一版未发布的 Escape 修复候选。默认 [`series`](series) 描述当前 `.4` 源码候选；源码候选不代表已经交付 runtime。
...
- `cx26.3-wine11.0-runeon.4` 是当前源码候选：继承 `.3`，并改进受管理的 Steam 启动验证。源码 Release 和 Runeon runtime 的独立门禁完成前，不能描述为用户已经获得。
