...
現在の `.9` ソース候補は配布済みの `.8` ベースラインと製品パッチを保持します。[Wine 11.16/11.17 の対象限定レビュー](BACKPORTS-11.16-11.17.ja.md)で 13 の上流コミットを含む 10 パッチファイルを追加しますが、完全監査の境界は `wine-11.15` のままです。作業中の候補 series に含まれることは、以下の採用・リリースゲートが合格したことを意味しません。別途検証した runtime のリリースで状態が変わるまで、配布済みソースは `.8` のままにします。
