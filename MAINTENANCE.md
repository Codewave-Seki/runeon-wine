...
The active `.9` source candidate preserves the shipped `.8` baseline and its product patches. Its [targeted Wine 11.16/11.17 review](BACKPORTS-11.16-11.17.md) adds 10 patch files carrying 13 upstream commits; it does not extend the complete audit beyond `wine-11.15`. Presence in the working candidate series does not mean that the acceptance and release gates below have passed. Keep `.8` as the shipped source until a separately validated runtime release changes that status.
