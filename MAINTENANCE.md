...
Current Dev source: [`cx26.3-wine11.0-runeon.10`](FLSGETVALUE2.md) (Pre-release), paired with seed `2026.09.22` and MoltenVK `1.4.2`, App gate `>=1.8 (0)`. Select its independent `patchsets/` definition explicitly; the default series remains `.9`. Validation: Dev artifact, API integration, authenticated feed/ticket and full-download SHA-256 checks completed. Dev App `1.8 (4)` is published; Production App remains `1.8 (3)`. Real Steam/game and App acceptance is still pending the user’s Dev validation.
