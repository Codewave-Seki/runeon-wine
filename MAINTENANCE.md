...
The active `.9` patch set preserves the `.8` baseline and its product patches. Its [targeted Wine 11.16/11.17 review](BACKPORTS-11.16-11.17.md) adds 10 patch files carrying 13 upstream commits; it does not extend the complete audit beyond `wine-11.15`. Dev seed `2026.09.13` pairs `.9` with MoltenVK `1.4.2`. Automated seed build, signing, API regression and Dev distribution verification are complete. Hands-on Steam/game acceptance remains pending and will use a Dev build of the current Runeon App source, version `1.8 (3)`. Existing published App installers are unchanged. `.9` remains a Pre-release; Production and the latest stable source remain `.8` with seed `2026.09.06`. The remaining product and Production release gates below still apply.
